...
## Closing Details
My matrix struct(s) were definitely what I spent most of my time on in this project. It uses CRTP, which I think I implemented by accident when using templates for all my matrix structs. I made lazy evaluation, using wrappers for different functions like matrix adding and subtracting. Matrix multiplication unfortunately doesn't really benefit from the lazy evaluation, as you would have to compute a single item times a whole column every time you look up a number. Element wise multiplication works fine with this. It is a lot of fun to optimize this stuff. Now, although the matrix struct(s) are what I spent the most time on, they were definitely not the most difficult. However, I had a lot of fun making that part.
